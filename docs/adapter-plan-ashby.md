# Ashby Adapter Implementation Plan

## Overview

Implement an `AshbyAdapter` that scrapes job listings from Ashby-powered company career pages via their GraphQL API.
...
```json
{
  ...
  "variables": {
    "organizationHostedJobsPageName": "zip"
  },
...
- Retry with exponential backoff on 429/503/502
...
**File:** `backend/src/adapters/ashby-adapter.ts`
...
function parseLocation(locationName: string): Location {
...
function parseJobType(employmentType?: string): Job['job_type'] {
...
**File:** `backend/src/adapters/__tests__/ashby-adapter.test.ts`

Tests covering:

1. **`parseLocation`** — "San Francisco, CA" → city/state, "Remote - US" → remote:true, empty → default
2. **`parseJobType`** — "Full-time" → 'full-time', "Contract" → 'contract', missing → 'full-time'
3. **`parseSeniority`** — title-based parsing
4. **`extractTags`** — keyword extraction from title
5. **`transformAshbyJob`** — full transform with mock posting
6. **`fetchOrgJobs`** — mock GraphQL response, verify jobs built correctly
7. **`fetchOrgJobs` retry** — mock 429 then 200, verify retry succeeds
...
### Task 4: Environment variable (optional)

```bash
# .env
ASHBY_ORGS=openai,anthropic,cohere
```
...
- [ ] `AshbyAdapter` implements `BoardAdapter` interface
- [ ] `npm test` passes with all adapter tests
- [ ] `npm run build` compiles without errors
- [ ] `GET /api/health` shows `ashby` in adapters list
- [ ] `POST /api/jobs/search` with `boards: ["ashby"]` returns jobs
- [ ] `healthCheck()` returns `healthy: true`
- [ ] At least 10 jobs fetched from a known org
- [ ] Retry logic works (429/503/502 → exponential backoff)
- [ ] User-Agent rotation on retries
- [ ] Jitter before requests (0.5-2.0s)
