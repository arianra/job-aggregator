...
**Phase 2 (Scraping):**
5. `POST /api/jobs/scrape` (trigger scrape)
6. `GET /api/jobs/scrape/:id` (scrape status)

**Phase 3 (Future):**
7. `POST /api/auth/login` (authentication)
8. `GET /api/profile` (user profile)
9. `GET /api/profile/matches` (scored jobs)
