...
## 6.5 Resume entity model & ontology (one or more resumes)

> **Update (2026-08-14):** The entity model and ontology below are **superseded in structure by
> ADR-0008** (Profile → Resume → ResumeVersion). Read ADR-0008 for the current model. This section
> is kept as the **document shape / field reference** (the `data` blob of a `ResumeVersion`), not as
> the DB topology.

The app stores **one or more `Resume`s** per user. Each `Resume` is the **source of truth** (structured
data, §3) and renders to the final **DOCX** (canonical) + derived **PDF**. **Import fills fields; it never
becomes the source of truth** — the internal model always wins.

```
Resume {
  id, ownerId, title, createdAt, updatedAt
  format: 'rezi-compact'            // chosen template from a small constrained set (§6.6)
  contact:    { name, email, phone, linkedin, website, country, state, city, visibility{} }
  summary:    string
  experience: [ { role, company, dates, location, bullets: string[], visible } ]
  education:  [ { degree, school, location, year, minor?, gpa?, extra? } ]
  skills:     { category: [skill, …] }                // ordered; ordered categories → format lines
  certifications: [ { title, issuer, year } ]         // optional section (O2)
  sections:   { order: string[], visibility: {} }     // add / drag-reorder / rename / delete-gated-by-confirm
  settings:   { fontSize, lineHeight, spacing, indent, divider, paper, typeface, color }  // fit controls
  ats:        { lastLint: { overall, categories, rules[] } }   // deterministic snapshot
}
```

- **Multi-resume:** a user can own many `Resume`s; each is independent (own fields, section set, settings,
  lint snapshot). A resume switcher surfaces them; **duplicate resume** is a deep copy.
- **Canonical artifact is an `output`:** the generated `.docx` (+ derived `.pdf`) are derived from this
  model, regenerated on save; keep latest + last-N history (O5). The DOCX/PDF on disk never supersede the
  structured data.
- **Import = best-effort prefill:** on upload of a PDF/DOCX, extract text (`mammoth`/`pdf.js`) + scanned
  detect → **Qwen parse** (ADR-0002) → map into contact/summary/experience/education/skills/certs.
  Unmappable fields are left blank and flagged for review. The uploaded file is then discardable — the
  parsed data populates a `Resume` the user edits.
- **UI — the shared “organizable group” (ADR-0007):** experience / education / skills / certifications
  all use one component offering **add · drag-reorder · rename · delete** where **delete always prompts
  for confirmation**. Skills are `category → [skills]`; each category renders one §2 format line.

## 6.6 DOCX formatting = a small set of templates

Formatting is **choosing a template**, not free-styling. A deliberately small set of common templates
(e.g. `rezi-compact`; later `classic` / `modern` / `a4-spacious`), each defining heading style, typeface,
spacing and paper defaults — all generated by `docx.js` under the §2 no-drift contract. The **fit controls**
(font size / line-height / spacing / indent / divider / paper / typeface — ADR-0007 §5) are the only knobs
*within* a template. **For the first release we ship exactly one template (`rezi-compact`)** and expand
the set later (ADR-0007 open item).
