...
export function parseSeniority(title: string): Job['seniority_level'] {
  ...
  if (lower.includes('entry') || lower.includes('junior') || lower.match(/\bjr\.?\b/)) return 'entry';
...
export function transformWorkdayJob(raw: WorkdayJob, tenant: WorkdayTenant): { job: Job; source: Source } {
