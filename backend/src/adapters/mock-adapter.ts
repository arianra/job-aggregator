...
export class MockAdapter implements BoardAdapter {
  ...
  async searchJobs(query: JobSearchQuery): Promise<AdapterResult> {
    ...
    if (query.title) {
      ...
      filteredJobs = filteredJobs.filter((j) => j.title.toLowerCase().includes(titleLower))
    }
    ...
    if (query.salaryMin !== undefined) {
      filteredJobs = filteredJobs.filter(
        (j) => j.salary_range && j.salary_range.min >= query.salaryMin!
      )
    }

    if (query.salaryMax !== undefined) {
      filteredJobs = filteredJobs.filter(
        (j) => j.salary_range && j.salary_range.max <= query.salaryMax!
      )
    }
