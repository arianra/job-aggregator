...
describe('Applications API', () => {
  ...
  describe('POST /api/applications', () => {
    it('saves a job by default', async () => {
      const res = await request(app).post('/api/applications').send({ job_id: 'job-1' })
    ...
    it('returns 404 if job does not exist', async () => {
      const res = await request(app).post('/api/applications').send({ job_id: 'nonexistent' })
    ...
    it('updates job status to applied when applying', async () => {
      await request(app).post('/api/applications').send({ job_id: 'job-1', status: 'applied' })
  ...
  describe('GET /api/applications', () => {
    ...
    it('filters by status', async () => {
      const res = await request(app).get('/api/applications').query({ status: 'applied' })
  ...
  describe('PUT /api/applications/:id', () => {
    ...
    it('updates application status', async () => {
      const res = await request(app).put(`/api/applications/${appId}`).send({ status: 'interview' })
    ...
    it('adds multiple notes over time', async () => {
      await request(app).put(`/api/applications/${appId}`).send({ note: 'First note' })

      await request(app).put(`/api/applications/${appId}`).send({ note: 'Second note' })
    ...
    it('validates status enum', async () => {
      const res = await request(app).put(`/api/applications/${appId}`).send({ status: 'flying' })
  ...
  describe('DELETE /api/applications/:id', () => {
    it('deletes an application', async () => {
      const create = await request(app).post('/api/applications').send({ job_id: 'job-1' })
