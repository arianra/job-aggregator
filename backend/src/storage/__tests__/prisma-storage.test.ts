...
const testDatabaseUrl =
  ...
  'postgresql://job_aggregator:dev_password_local_only@localhost:5432/job_aggregator_test'
...
function makeProfile(overrides: Partial<Profile> = {}): Profile {
  return {
    ...
    skills: [{ name: 'TypeScript', proficiency: 'expert', years: 5, category: 'language' }],
    experience: [
      {
        company: 'OldCorp',
        title: 'Engineer',
...
describe('PrismaStorage', () => {
  ...
  describe('Jobs', () => {
    ...
    it('saves job with all fields', async () => {
      ...
      await storage.saveJob(job)
      const retrieved = await storage.getJob(job.id)
      expect(retrieved!.salary_range).toEqual({
        min: 120000,
        max: 180000,
        currency: 'USD',
        period: 'annual',
    ...
    it('post-filters by location', async () => {
      ...
      await storage.saveJob(
        makeJob({
          id: 'j2',
          location: { city: 'Seattle', state: 'WA', country: 'USA', remote: false },
        })
      )
    ...
    it('post-filters by remote', async () => {
      await storage.saveJob(
        makeJob({ id: 'j1', is_remote: true, location: { country: 'USA', remote: true } })
      )
