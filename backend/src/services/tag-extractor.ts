...
export async function tagJobsWithSkills(
  ...
  const profileSkillNames = new Set(profile.skills.map((s) => normalizeSkill(s.name)))
 ...
 */
function extractFallbackTags(job: Job, profileSkills?: Set<string>): string[] {
  const text = getJobText(job).toLowerCase()

  // If we have profile skills, only match against those
  const keywords = profileSkills ? Array.from(profileSkills) : FALLBACK_KEYWORDS
