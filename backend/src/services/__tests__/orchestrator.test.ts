...
describe('Orchestrator', () => {
  ...
  describe('searchAll', () => {
    it('runs all adapters, deduplicates, and persists', async () => {
      ...
      const linkedin = new MockAdapter(
        'linkedin',
        'LinkedIn',
        [job2],
        [makeSource('job-2', 'linkedin')]
      )
    ...
    it('deduplicates identical jobs across adapters', async () => {
      ...
      const indeed = new MockAdapter(
        'indeed',
        'Indeed',
        [jobShared],
        [makeSource('same-job', 'indeed')]
      )
      const linkedin = new MockAdapter(
        'linkedin',
        'LinkedIn',
        [jobShared],
        [makeSource('same-job', 'linkedin')]
      )
    ...
    it('does not deduplicate when existing storage is empty', async () => {
      ...
      const adapter = new MockAdapter('indeed', 'Indeed', jobs, [
