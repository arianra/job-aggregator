import type { Job, Profile, Skill, Match, MatchDimensions, DimensionScore } from '@job-aggregator/shared';
...
function scoreSkills(profileSkills: Skill[], job: Job): DimensionScore {
  ...
  for (const ps of profileSkills) {
    ...
    if (jobSkillNames.has(normalized)) {
      ...
      const profBonus = { beginner: 0.5, intermediate: 0.75, advanced: 1.0, expert: 1.25 }[ps.proficiency] ?? 1.0;
  ...
  const coverage = profileSkills.length > 0
    ? Math.min(1, (matched + bonusWeight * 0.5) / profileSkills.length)
    : 0;
...
function scoreExperience(
  ...
  const seniorityMap: Record<string, number> = {
    intern: 0, entry: 1, mid: 3, senior: 5, lead: 7, manager: 8, director: 10, vp: 12, executive: 15,
  };
