...
export function generateFingerprint(job: {
  ...
  const location = normalize([job.location.city, job.location.state, job.location.country].filter(Boolean).join(' '));
...
export async function deduplicateJobs(
  ...
  logger.info(`[dedup] ${deduped} duplicates found, ${merged} merged, ${saved.length} new jobs saved`);
...
function mergeJob(canonical: Job, duplicate: Job): Partial<Job> {
  ...
  if (duplicate.salary_range?.min && duplicate.salary_range?.max) {
    if (
      !canonical.salary_range?.min ||
      !canonical.salary_range?.max
    ) {
